...
#if defined(__loongarch_lp64) || defined(__loongarch__)
	result = "LOONGARCH64";
#endif
...
String* CPU_model (CPU *self) 
{
	...
	if (result) {
        ...
        char *result = execute_and_return_first_line("grep -i '^Model Name.*:' /proc/cpuinfo | sed 's/^.*: //' | tail -1");
        if (result) {
            return String_withCString(result);
        }
    #endif
...
#endif
#if defined(__loongarch_lp64) || defined(__loongarch__)
	return String_withCString ("loongarch64");
#endif
