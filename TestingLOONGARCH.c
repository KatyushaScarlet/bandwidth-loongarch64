...
#include "TestingLOONGARCH.h"
#include "Console.h"
#include "routines.h"

TestingLOONGARCHClass* _TestingLOONGARCHClass = NULL;
...
static void TestingLOONGARCH_destroy (Any* self)
{
	if (!self)
		return;
	verifyCorrectClass(self,TestingLOONGARCH);
}

static void TestingLOONGARCH_describe (TestingLOONGARCH* self, FILE *outputFile)
{
        if (!self)
                return;
        verifyCorrectClass(self,TestingLOONGARCH);
...
//----------------------------------------------------------------------------
// Name:	TestingLOONGARCH_write
// Purpose:	Performs write on chunk of memory of specified size.
//----------------------------------------------------------------------------
long TestingLOONGARCH_write (TestingLOONGARCH *self, unsigned long size, TestingMode mode, bool random)
{
...
//----------------------------------------------------------------------------
// Name:	TestingLOONGARCH_read
// Purpose:	Performs sequential read on chunk of memory of specified size.
//----------------------------------------------------------------------------
long TestingLOONGARCH_read (TestingLOONGARCH *self, unsigned long size, TestingMode mode, bool random)
{
	if (size == CHECK_WHETHER_SUPPORTED) {
		switch (mode) {
		case SIZE_MAIN_REGISTER:
...
//----------------------------------------------------------------------------
// Name:	TestingLOONGARCH_copy
// Purpose:	Performs sequential memory copy.
//----------------------------------------------------------------------------
long TestingLOONGARCH_copy (TestingLOONGARCH *self, unsigned long size, TestingMode mode)
{
	if (size == CHECK_WHETHER_SUPPORTED) {
		switch (mode) {
		case SIZE_MAIN_REGISTER:
...
TestingLOONGARCHClass* TestingLOONGARCHClass_init (TestingLOONGARCHClass *class)
{
	SET_SUPERCLASS(Testing);

	SET_OVERRIDDEN_METHOD_POINTER(TestingLOONGARCH,describe);
	SET_OVERRIDDEN_METHOD_POINTER(TestingLOONGARCH,destroy);

        SET_INHERITED_METHOD_POINTER(TestingLOONGARCH,Object,print);
        SET_INHERITED_METHOD_POINTER(TestingLOONGARCH,Object,equals);

	SET_METHOD_POINTER(TestingLOONGARCH,read);
	SET_METHOD_POINTER(TestingLOONGARCH,write);
	SET_METHOD_POINTER(TestingLOONGARCH,copy);
	...
	SET_ABSTRACT_METHOD_POINTER(registerToVector64);
        ...
        VALIDATE_CLASS_STRUCT(_TestingLOONGARCHClass);
	return _TestingLOONGARCHClass;
}

TestingLOONGARCH *TestingLOONGARCH_init (TestingLOONGARCH *self)
{
	ENSURE_CLASS_READY(TestingLOONGARCH);

        Testing_init ((Testing*) self);

        self->is_a = _TestingLOONGARCHClass;
